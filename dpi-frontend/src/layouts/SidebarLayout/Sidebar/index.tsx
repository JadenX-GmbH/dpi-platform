...
import CreateEditProjectModal from 'src/components/Modals/ProjectModal';
...
function Sidebar() {
  ...
  return (
    <>
      ...
      >
        <Scrollbar>
          ...
          {(user['https://auth.dpi.com/roles'][0] === 'Receiver' || user['https://auth.dpi.com/roles'][0] === 'Funder') && (
            <Box p={2} pb={0}>
              <Button onClick={() => setOpenCreateProjectModal(true)} variant="contained" color="primary" size="medium" fullWidth startIcon={<AddCircleOutlineIcon fontSize="small" />}>
                Create Project
              </Button>
            </Box>
          )}
      ...
      <CreateEditProjectModal openedModal={openCreateProjectModal} closeModal={handleCloseCreateProjectModal} />
    </>
  );
}
